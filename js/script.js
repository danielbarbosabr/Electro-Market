...
// Contexto de "responder a um post da Comunidade dentro da conversa" é preenchido
// por startPostReplyInChat() e consumido por sendDirectChatMessage() (ver abaixo).
...
const TERMS_SECTIONS = [
    ...
    { icon: 'headset',       title: '15. Contato e Suporte', id: 'ts15', html: '<p>Em caso de dúvidas, sugestões ou solicitações relacionadas a estes Termos de Uso, entre em contato conosco:</p><ul><li><strong>E-mail:</strong> dannybarbosalimabr@gmail.com</li><li><strong>Suporte na plataforma:</strong> utilize a seção "Falar com o Suporte" disponível no menu</li></ul><p>Nosso time responderá em até 48 horas úteis.</p>' }
];
...
const PRIVACY_SECTIONS = [
    ...
    { icon: 'envelope',     title: '10. Contato', id: 'ps10', html: '<p>Em caso de dúvidas sobre esta Política de Privacidade ou sobre o tratamento dos seus dados:</p><ul><li><strong>E-mail:</strong> dannybarbosalimabr@gmail.com</li><li><strong>Suporte:</strong> utilize a seção "Falar com o Suporte" na plataforma</li></ul>' }
];
...
function bootstrapApp() {
    ...
    document.getElementById('profileEditForm')?.addEventListener('submit', async (e) => {
        ...
        // Valida o CPF antes de seguir à mesma validação usada no cadastro,
        // pra não permitir salvar um CPF incompleto/inválido pelo perfil.
        const cpfInputEl = document.getElementById('editCPF');
        const novoCpf = cpfInputEl ? cpfInputEl.value.replace(/\D/g, '') : (user.cpf || '');
        if (cpfInputEl && !validarCPF(novoCpf)) {
...
ElectroMarket - Plataforma de E-commerce
  `.trim();
  const mailto = `mailto:dannybarbosalimabr@gmail.com?subject=${encodeURIComponent('[Suporte ElectroMarket] Esqueci minha senha')}&body=${encodeURIComponent(body)}`;
...
window.renderMsgBubble = function(msg, index, opts = {}) {
    ...
    // Comentário feito numa Thread da Comunidade, espelhado aqui na conversa
    // privada entre autor da Thread e quem comentou (ver twMirrorReplyToDirectChat).
    if (msg.type === 'thread_comment_ref') {
        const isMeTw = msg.senderId === userId;
        const cleanTextTw = window.stripLegacyEmoji?.(msg.text || '') ?? (msg.text || '');
        const avatarForThemTw = msg.senderAvatar || resolveSenderAvatar?.(msg) || partnerAvatar || '';
        const labelTw = isMeTw ? 'Comentário na Thread' : 'Comentário na sua Thread';
        return `
        <div class="msg-row ${isMeTw ? 'is-me' : 'is-them'}" data-msg-index="${index}">
            ${!isMeTw ? `<img class="msg-avatar" src="${avatarForThemTw}" referrerpolicy="no-referrer" onerror="this.onerror=null;this.style.display='none'">` : ''}
            <div class="msg-bubble ${isMeTw ? 'is-me' : 'is-them'} msg-single tw-comment-ref-bubble" style="cursor:pointer;" onclick="window.exitWaOrdersView();window.openCommunityThread('${msg.threadId}')" title="Ver Thread original">
                <div class="msg-sender" style="margin-bottom:4px;"><i class="bi bi-chat-square-text-fill me-1"></i>${labelTw}</div>
                ${msg.image ? `<img src="${msg.image}" class="img-fluid rounded mb-2" referrerpolicy="no-referrer" style="max-width:200px;">` : ''}
                ${cleanTextTw ? `<div class="chat-bubble-text" style="white-space:pre-wrap;">${window.formatLinks?.(cleanTextTw) ?? cleanTextTw}</div>` : ''}
                <div style="font-size:0.7rem;opacity:0.7;margin-top:4px;"><i class="bi bi-box-arrow-up-right me-1"></i>Ver na Thread</div>
            </div>
            ${isMeTw ? `<img class="msg-avatar" src="${myAvatar}" referrerpolicy="no-referrer" onerror="this.onerror=null;this.style.display='none'">` : ''}
        </div>`;
    }
